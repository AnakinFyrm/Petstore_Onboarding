...
`pet-agent` is a Model Context Protocol tool server built on FastMCP. The console script
`pet-agent` starts the server defined in `pet_agent.server` over stdio, so an MCP client owns
