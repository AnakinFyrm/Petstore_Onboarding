...
`pet-agent` is a Model Context Protocol tool server built on FastMCP. `src/pet_agent/server.py`
...
| `make run` | `uv run pet-agent` — the MCP server over stdio (blocks; a client drives it). |
