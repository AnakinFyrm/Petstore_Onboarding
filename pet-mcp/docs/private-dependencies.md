...
```bash
gh secret set GH_DEP_TOKEN -R fyrm-ai/pet-mcp
...
docker build --secret id=github_token,env=GH_TOKEN -t pet-mcp .
```
...
```bash
gh api -X PUT repos/fyrm-ai/pet-mcp/vulnerability-alerts
gh api -X PUT repos/fyrm-ai/pet-mcp/automated-security-fixes
```
