...
Runbook for `pet-mcp`: how it ships, how to tell whether it is healthy, and what to check
...
like `uv run pet-mcp` with the working directory set to this checkout, or
`docker run --rm -i ghcr.io/fyrm-ai/pet-mcp:X.Y.Z` for a pinned image.
...
re-syncing inside the container, and `CMD` is the `pet-mcp` console script.
...
| Trigger | Tags |
|---|---|
| merge to `main` | `ghcr.io/fyrm-ai/pet-mcp:dev` and `:sha-<commit>` |
...
the process exits. To check a build by hand, run `uv run pet-mcp` and confirm it stays up
