...
```bash
gh secret set GH_DEP_TOKEN -R fyrm-ai/pet-agent
...
The `Dockerfile` installs `git` and accepts the token as a BuildKit secret that
is used only during `uv sync` and never written into an image layer:
...
For all-public dependencies you can build normally; the secret is optional.
...
```bash
gh api -X PUT repos/fyrm-ai/pet-agent/vulnerability-alerts
gh api -X PUT repos/fyrm-ai/pet-agent/automated-security-fixes
```
